...
import { getProductById } from "../../_lib/data-service"; // Adjust path
...
const ProductDetails = ({ productId, initialProduct }: Props) => {
  ...
  const { data, isLoading, isError, error } = useQuery<Product | null, Error>({ // Explicitly type Error
  ...
  if (isProductIdInvalid) {
    return <div className="text-center text-red-500 p-10">Invalid Product ID provided.</div>;
  }
  ...
  if (isError && !data) {
    return (
      <div className="flex flex-col justify-center items-center h-64 gap-4 p-4">
        <p className="text-xl text-red-600 font-semibold">Error Loading Product</p>
        {/* Optionally display the error message */}
        <p className="text-sm text-gray-600 bg-red-50 p-2 border border-red-200 rounded">{error?.message ?? 'An unknown error occurred.'}</p>
      </div>
    );
  }
  ...
  return <div className="text-center text-gray-500 p-10">Loading or unexpected state...</div>;
};
