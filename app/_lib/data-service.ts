...
const getCategoryName = (categoriesData: RawProductData['categories']): Category | null => {
  if (!categoriesData) return null;
  // Handle both single object and array (Supabase might return array even for single)
  const category = Array.isArray(categoriesData) ? categoriesData[0] : categoriesData;
  return category && typeof category.name === 'string' ? { name: category.name } : null;
}

// Helper to safely process images
const processImages = (imagesData: RawProductData['product_images']): ProductImage[] | null => {
  ...
  const validImages = imagesData.filter(
    (img): img is ProductImage => // Type predicate for stricter checking
      img && typeof img.image_url === 'string' && typeof img.is_primary === 'boolean'
  );
...
export const getProducts = async function(): Promise<Product[]> {
  ...
  const products: Product[] = data.map((item: RawProductData): Product => ({
    // Ensure ID is treated as number
    id: typeof item.id === 'number' ? item.id : parseInt(String(item.id), 10) || 0, // Robust ID parsing
...
// Accepts number ID
export const getProductById = async function(id: number): Promise<Product | null> {
   // Initial validation
   if (isNaN(id) || id <= 0) {
  ...
  if (!data) {
   ...
   const product: Product = {
     // Ensure ID is treated as number
     id: typeof rawData.id === 'number' ? rawData.id : parseInt(String(rawData.id), 10) || 0,
   ...
   if (product.id <= 0) {
       console.error(`Failed to parse valid ID for fetched product data:`, rawData);
       throw new Error(`Workspaceed product data resulted in invalid ID for request ${id}`);
   }
